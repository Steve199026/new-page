...
; (function () {
	...
	nc.global_validation = {
		...
		msgpos: 'normal',
		msg: {
			mail: { email: "Bitte gebe eine gültige Email-Adresse ein" }
		},
		subscribe_successMsg: "Du wurdest erfolgreich in unsere Newsletterliste aufgenommen. Folge dem Ballon!",
		form_successMsg: "Du wurdest erfolgreich in unsere Newsletterliste aufgenommen. Folge dem Ballon!",

		successMsg: "",
		errorMsg: "Upps! Es scheint zu einem Fehler gekommen zu sein. Versuche es später nochmal."
	}
			...
			$.ajax({
				...
				success: function (data) {
					if (data.status == 'error') {
                        ...
                        swal("Fehler!", data.type, "error");
                        $btn.button('reset');
                        nc.resetForm($form);
                    }
					else if (data.status == 'info') {
                        swal("Info", data.type, "info");
                        $btn.button('reset');
                        nc.resetForm($form);
					} else {
	...
	nc.countdown = function (obj) {
		...
		countdownHtml += '	<!-- DAYS -->';
		countdownHtml += '	<div class="dash days_dash"> TAGE';
		...
		countdownHtml += '	</div>';
		countdownHtml += '	<!-- HOURS -->';
		countdownHtml += '	<div class="dash hours_dash"> STUNDEN';
		countdownHtml += '		<div class="inner-dash flex-cc">';
		countdownHtml += '			<div class="digit">0</div>';
		countdownHtml += '			<div class="digit">0</div>';
		countdownHtml += '		</div>';
		...
		countdownHtml += '	<div class="dash minutes_dash"> MINUTEN';
		countdownHtml += '		<div class="inner-dash flex-cc">';
		countdownHtml += '			<div class="digit">0</div>';
		countdownHtml += '			<div class="digit">0</div>';
		countdownHtml += '		</div>';
		...
		countdownHtml += '	<div class="dash seconds_dash"> SEKUNDEN';
		countdownHtml += '		<div class="inner-dash flex-cc">';
		countdownHtml += '			<div class="digit">0</div>';
		countdownHtml += '			<div class="digit">0</div>';
		countdownHtml += '		</div>';
		...
		if ($o.r) {
			...
			if ($o.gmapwidget) {
				var social_script_arr = [
					"https://maps.googleapis.com/maps/api/js?sensor=false&libraries=places&key=AIzaSyCA4hxJm9F-6p-OSreubU3W5Vsq-nQ1LZ0",
