...
if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST["email"])) {
	...
	$firstname = isset($_POST["fname"]["val"]) ? $_POST["fname"]["val"] : '';
	$lastname = isset($_POST["lname"]["val"]) ? $_POST["lname"]["val"] : '';
	...
	if(filter_var($email, FILTER_VALIDATE_EMAIL)) {
		...
		elseif ($STORE_MODE == "mailchimp") {
			...
			// echo "<pre>".print_r(array('email'=>$email, 'fname'=>$firstname, 'lname'=>$lastname))."</pre>";     
			...
			if($MailChimp->success()) {
				if ($result["email_address"] == $email) {
					...
					errorlog("mailchimp", json_encode($result));
				}
			// ERROR SENDING
			} else {
                if($result["title"] == "Member Exists" ){
                    echo json_encode(array(
                        "status" => "info",
                        "type"   => "Du bist bereits in unserem Newsletter eingetragen."
                    ));
			    }else{
                    echo json_encode(array(
                        "status" => "error",
                        "type"   => "Du konntest nicht für den Newsletter eingetragen werden. Versuche es später nocheinmal."
                    ));
